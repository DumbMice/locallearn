...
network = PC(etol=1., beta=50., max_iter=1000)
network.addlayerednodes(4, False,data_init=torch.nn.init.zeros_)
...
network.connect(0,1,Sequential(Conv2d(3, 32, kernel_size=(3, 3),padding=1,bias=True),MaxPool2d(2,return_indices=True),Tanh()))
network.connect(1,2,Sequential(Conv2d(32, 64, kernel_size=(3, 3),padding=1,bias=True),MaxPool2d(2,return_indices=True),Tanh()))
# network.connect(2,3,Sequential(Conv2d(64, 128, kernel_size=(3, 3),padding=1,bias=True),MaxPool2d(2,return_indices=True),Tanh()))
# network.connect(3,2,Sequential(MaxUnpool2d(2),Conv2d(128, 64, kernel_size=(3, 3),padding=1,bias=True),Tanh()),scale=0.)
# network.connect(3,4,Sequential(Conv2d(128, 64, kernel_size=(3, 3),padding=1,bias=True),MaxPool2d(2,return_indices=True),Tanh()))
# network.connect(4,3,Sequential(MaxUnpool2d(2),Conv2d(64, 128, kernel_size=(3, 3),padding=1,bias=True),Tanh()),scale=0.)
network.connect(2,3,Sequential(Flatten(),Linear(4096,10,bias=True)))
network.connect(1,0,Sequential(MaxUnpool2d(2),Conv2d(32, 3, kernel_size=(3, 3),padding=1,bias=True),Tanh()),scale=0.)
network.connect(2,1,Sequential(MaxUnpool2d(2),Conv2d(64, 32, kernel_size=(3, 3),padding=1,bias=True),Tanh()),scale=0.)
network.connect(3,2,Sequential(Linear(10,4096,bias=True),Unflatten(1,[64,8,8])),scale=0.)
